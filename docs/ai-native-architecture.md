...
第一阶段原始边界不接入主动端口扫描、Nuclei、企业工商查询或外部威胁情报；当前实现已在后续阶段加入受控端口基线、隔离 Nuclei adapter、metadata-only GitHub 情报、exact CPE-backed NVD CVE 关联与 licensed registration provider contract。
