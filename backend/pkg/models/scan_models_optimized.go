...
// 优化后的数据模型 - 基于Linus审查建议，减少嵌套层级，提升查询性能

// Project - 扫描项目
type ProjectOptimized struct {
...
type ScanTarget struct {
	...
	ScanResults []ScanResultOptimized `json:"scan_results,omitempty" gorm:"foreignKey:TargetID"`
}
...
type ScanResultOptimized struct {
	...
	Vulnerabilities []VulnerabilityOptimized `json:"vulnerabilities,omitempty" gorm:"foreignKey:ScanResultID"`
	WebPaths       []WebPathOptimized       `json:"web_paths,omitempty" gorm:"foreignKey:ScanResultID"`
}

// Vulnerability - 漏洞信息
type VulnerabilityOptimized struct {
	ID           uint      `json:"id" gorm:"primaryKey"`
	...
	// 关联关系
	ScanResult ScanResultOptimized `json:"-" gorm:"foreignKey:ScanResultID"`
	WebPath    *WebPathOptimized   `json:"-" gorm:"foreignKey:WebPathID"`
}

// WebPath - Web路径（仅针对Web服务）
type WebPathOptimized struct {
	ID           uint      `json:"id" gorm:"primaryKey"`
	ScanResultID uint      `json:"scan_result_id" gorm:"not null;index"`
	...
	// 关联关系
	ScanResult      ScanResultOptimized      `json:"-" gorm:"foreignKey:ScanResultID"`
	Vulnerabilities []VulnerabilityOptimized `json:"vulnerabilities,omitempty" gorm:"foreignKey:WebPathID"`
}

// Technology - 技术栈（多对多关系）
type TechnologyOptimized struct {
	ID        uint      `json:"id" gorm:"primaryKey"`
	Name      string    `json:"name" gorm:"not null;uniqueIndex;size:100"`
...
type ScanResultTechnology struct {
	...
	// 关联关系
	ScanResult ScanResultOptimized  `json:"-" gorm:"foreignKey:ScanResultID"`
	Technology TechnologyOptimized  `json:"-" gorm:"foreignKey:TechnologyID"`
}

// 优化后的查询视图结构
type ProjectStatsOptimized struct {
...
func (sr *ScanResultOptimized) GetServiceSignature() string {
...
func (v *VulnerabilityOptimized) IsCritical() bool {
	return v.Severity == "critical"
}

func (v *VulnerabilityOptimized) IsOpen() bool {
