...
// 优化后的ScanDAO - 基于Linus审查建议，消除N+1查询，使用高效的JOIN查询

type ScanDAOOptimized struct {
	db *gorm.DB
}

func NewScanDAOOptimized(db *gorm.DB) *ScanDAOOptimized {
	return &ScanDAOOptimized{db: db}
}

// Project 管理 - 保持简单
func (d *ScanDAOOptimized) CreateProject(project *models.ProjectOptimized) error {
	return d.db.Create(project).Error
}

func (d *ScanDAOOptimized) GetProjectByID(id uint) (*models.ProjectOptimized, error) {
	var project models.ProjectOptimized
	err := d.db.First(&project, id).Error
	return &project, err
}

func (d *ScanDAOOptimized) GetProjectByName(name string) (*models.ProjectOptimized, error) {
	var project models.ProjectOptimized
	err := d.db.Where("name = ?", name).First(&project).Error
	return &project, err
}

func (d *ScanDAOOptimized) ListProjects() ([]models.ProjectOptimized, error) {
	var projects []models.ProjectOptimized
	err := d.db.Find(&projects).Error
	return projects, err
}

func (d *ScanDAOOptimized) DeleteProject(id uint) error {
	return d.db.Delete(&models.ProjectOptimized{}, id).Error
}

// 高效的项目详情查询 - 单次JOIN代替多次Preload
func (d *ScanDAOOptimized) GetProjectDetails(projectID uint) (*models.ProjectStatsOptimized, []models.ScanTarget, error) {
	// 获取项目基本信息和统计
	stats, err := d.GetProjectStatsOptimized(projectID)
	if err != nil {
		return nil, nil, err
	}
...
func (d *ScanDAOOptimized) GetProjectStatsOptimized(projectID uint) (*models.ProjectStatsOptimized, error) {
	var stats models.ProjectStatsOptimized
	...
	err := d.db.Raw(`
		...
		FROM scan_targets t
		LEFT JOIN scan_result_optimizeds sr ON t.id = sr.target_id
		WHERE t.project_id = ?
	...
	// 漏洞统计
	vulnStats, err := d.GetVulnerabilityStatsOptimized(projectID)
	if err != nil {
		return nil, err
	}
	...
	d.db.Raw(`
		...
		FROM scan_result_optimizeds sr
...
// 高效的漏洞统计
func (d *ScanDAOOptimized) GetVulnerabilityStatsOptimized(projectID uint) (map[string]int, error) {
...
func (d *ScanDAOOptimized) ImportScanData(data *models.ScanDataImport) error {
	return d.db.Transaction(func(tx *gorm.DB) error {
		...
		// 第三遍：批量创建扫描结果
		var vulnerabilities []models.VulnerabilityOptimized
			...
			for _, portData := range targetData.Ports {
				...
				if portData.Service != nil {
					...
					for _, pathData := range portData.Service.WebPaths {
						webPath := models.WebPathOptimized{
							ScanResultID: scanResult.ID,
							Path:         pathData.Path,
					...
					for _, techData := range portData.Service.Technologies {
						// 查找或创建技术
						tech := models.TechnologyOptimized{
							Name:     techData.Name,
...
// 高效的漏洞查询
func (d *ScanDAOOptimized) GetVulnerabilities(projectID uint, filters map[string]interface{}) ([]models.VulnerabilityOptimized, error) {
	query := d.db.Table("vulnerability_optimizeds v").
		Select("v.*, sr.port, sr.service_name, t.address, t.type").
		Joins("JOIN scan_result_optimizeds sr ON v.scan_result_id = sr.id").
...
// 构建层次结构视图（用于前端显示）
func (d *ScanDAOOptimized) GetProjectHierarchy(projectID uint) ([]models.ScanTarget, error) {
	var targets []models.ScanTarget
