...
type Website = { id: string; service_id: string; url: string; status_code: number; title: string; server: string; content_type: string; content_sha256: string; fingerprints: TechnologyFingerprint[]; discovered_paths: string[]; first_seen_at: Stamp; last_seen_at: Stamp }
...
function WebsiteTable({ websites }: { websites: Website[] }) {
  ...
  return <TableContainer><Table size="lg">
    ...
    <TableBody>{websites.map((website) => <TableRow key={website.id}>
      <TableCell><strong>{website.url}</strong><code>{shortId(website.content_sha256)} · {website.discovered_paths.length} crawled paths</code></TableCell>
